# MyLeetcode
All the questions I go through and my approach and understanding of each question.

# LeetCode Solutions (C++)

## 📊 Progress
- Total Solved: 30
- Language: C++

## 📁 Problems

| # | Problem | Topic | Difficulty | Solution |
|---|--------|------|------------|----------|
| 1 | Two Sum | Hashing | Easy | [Link](Hashing/001_TwoSum.cpp) |
| 49 | Group Anagrams | Hashing | Medium | [Link](Hashing/049_GroupAnagrams.cpp) |
| 217 | Contains Duplicate | Hashing | Easy | [Link](Hashing/217_ContainsDuplicates.cpp) |
| 242 | Valid Anagram | Hashing | Easy | [Link](Hashing/242_ValidAnagrams.cpp) |
| 26 | Remove Duplicates from Sorted Array | Arrays | Easy | [Link](Arrays/026_RemoveDuplicatesFromSortedArray.cpp) |
| 53 | Maximum Subarray | Arrays | Medium | [Link](Arrays/053_MaximumSubarray.cpp) |
| 66 | Plus One | Arrays | Easy | [Link](Arrays/066_PlusOne.cpp) |
| 1752 | Check if Array Is Sorted and Rotated | Arrays | Easy | [Link](Arrays/1752_CheckIfArrayIsSortedAndRotated.cpp) |
| 238 | Product of Array Except Self | Arrays | Medium | [Link](Arrays/238_ProductOfArrayExceptSelf.cpp) |
| 269 | Missing Number | Arrays | Easy | [Link](Arrays/269_MissingNumber.cpp) |
| 283 | Move Zeroes | Arrays | Easy | [Link](Arrays/283_MoveZeroes.cpp) |
| 70 | Climbing Stairs | 1-D DP | Easy | [Link](1-D_DP/070_ClimbingStairs.cpp) |
| 152 | Maximum Product Subarray | 1-D DP | Medium | [Link](1-D_DP/152_MaximumProductSubarray.cpp) |
| 300 | Longest Increasing Subsequence | 1-D DP | Medium | [Link](1-D_DP/300_LongestIncreasingSubsequence.cpp) |
| 322 | Coin Change | 1-D DP | Medium | [Link](1-D_DP/322_CoinChange.cpp) |
| 69 | Sqrt(x) | Binary Search | Easy | [Link](Binary_Search/069_Sqrt(x).cpp) |
| 33 | Search in Rotated Sorted Array | Binary Search | Medium | [Link](Binary_Search/033_SearchInRotatedSortedArray.cpp) |
| 74 | Search a 2D Matrix | Binary Search | Medium | [Link](Binary_Search/074_SearchA2DMatrix.cpp) |
| 153 | Find Minimum in Rotated Sorted Array | Binary Search | Medium | [Link](Binary_Search/153_FindMinimumInRotatedSortedArray.cpp) |
| 704 | Binary Search | Binary Search | Easy | [Link](Binary_Search/704_BinarySearch.cpp) |
| 136 | Single Number | Bit Manipulation | Easy | [Link](Bit_Manipulation/136_SingleNumber.cpp) |
| 190 | Reverse Bits | Bit Manipulation | Easy | [Link](Bit_Manipulation/190_ReverseBits.cpp) |
| 191 | Number of 1 Bits | Bit Manipulation | Easy | [Link](Bit_Manipulation/191_NumberOf1Bits.cpp) |
| 338 | Counting Bits | Bit Manipulation | Easy | [Link](Bit_Manipulation/338_CountingBits.cpp) |
| 9 | Palindrome Number | Math | Easy | [Link](Math/009_PalindromeNumber.cpp) |
| 11 | Container With Most Water | Two Pointers | Medium | [Link](Two_Pointers/011_ContainerWithMostWater.cpp) |
| 15 | 3Sum | Two Pointers | Medium | [Link](Two_Pointers/015_3Sum.cpp) |
| 125 | Valid Palindrome | Two Pointers | Easy | [Link](Two_Pointers/125_ValidPalindrome.cpp) |
| 167 | Two Sum II – Input Array Is Sorted | Two Pointers | Medium | [Link](Two_Pointers/167_TwoSum2InputArrayIsSorted.cpp) |
| 121 | Best Time to Buy and Sell Stock | Two Pointers | Easy | [Link](Two_Pointers/BestTimeToBuyAndSellStock.cpp) |
